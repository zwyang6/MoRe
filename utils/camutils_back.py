...
def assign_csc_tags(images, roi_mask=None, crop_num=8, crop_size=96, threshold=0.2, num_class=20):

    crops = []
    b, c, h, w = images.shape

    temp_crops = torch.zeros(size=(b, crop_num, c, crop_size, crop_size)).to(images.device)
    flags = torch.ones(size=(b, crop_num+2, num_class)).to(images.device)
    margin = crop_size//2
