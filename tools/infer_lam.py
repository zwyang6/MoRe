...
parser.add_argument("--model_path", default="/data/PROJECTS/MoRe_2024/代码复现/MoRe_实验室复现/00_exp/VOC/00_sota/checkpoints/model_iter_sota.pth", type=str, help="model_path")
...
def _validate(pid,model=None, dataset=None, args=None):
    ...
    with torch.no_grad(), torch.cuda.device(0):
        ...
        for idx, data in tqdm(enumerate(data_loader), total=len(data_loader), ncols=100, ascii=" >="):
            ...
            ###
            if args.cam2seg:
                cls, segs, _x4, cls_aux, attri_tokens_, score_maps,= model(inputs,with_gcr=args.with_gcr)
