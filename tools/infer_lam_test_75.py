...
from utils.camutils import cam_to_label, get_valid_cam, multi_scale_lam2
...
parser.add_argument("--refine_with_multiscale", default=True, type=bool, help="refine_cam_with_multiscale")
parser.add_argument("--cam2seg", default=True, type=bool, help="take cam as seg")
parser.add_argument("--clspreds_from_mct", default='xxx',type=str, help="pretrain url")
parser.add_argument("--crf_post", default=True, type=bool, help="take cam as seg")
...
parser.add_argument("--save_cam", default=True, type=bool, help="save the cam figs")
    ...
    with torch.no_grad(), torch.cuda.device(0):
        ...
        for idx, data in tqdm(enumerate(data_loader), total=len(data_loader), ncols=100, ascii=" >="):
            ...
            if args.cam2seg:
                ...
                for id in cls_idx[1:]:
                    if (torch.argmax(segs, dim=1)==id).sum() < 4:
                        cls_pred_from_seg[0,id-1] = 0

                ## cls_pred or cls_pred_from_seg
                cls_label = cls_pred_from_seg if cls_idx.sum() > 0 else cls_pred
            ...
            if args.refine_with_multiscale:
                _cams = multi_scale_lam2(model, inputs, [1.0,0.5,0.75,1.25,1.5,1.75,2.0])
            else:
                _cams = multi_scale_lam2(model, inputs, [1.0])
            ...
            if args.crf_post:
                ...
                np.save(args.logits_dir + "/" + name[0] + '.npy', {"valid_lam":valid_lam.cpu().numpy(),"keys":keys.cpu().numpy(),"keys_gt":keys_gt.cpu().numpy()})

    logging.info('MASK/LAM_score:')
    cam_score = evaluate.scores(gts, cams)
    metrics_tab = format_tabs([cam_score], ["lam"], cat_list=voc.class_list)
    logging.info("\n"+metrics_tab)
...
def crf_proc():
    ...
    def _job(i):
        ...
        # imageio.imsave(args.segs_dir + "/" + name + ".png", np.squeeze(pred).numpy().astype(np.uint8))
        # imageio.imsave(args.segs_rgb_dir + "/" + name + ".png", imutils.encode_cmap(np.squeeze(pred.numpy())).astype(np.uint8))
        imageio.imsave(args.segs_crf_rgb_dir + "/" + name + ".png", imutils.encode_cmap(np.squeeze(pred_crf)).astype(np.uint8))
...
if __name__ == "__main__":
    ...
    if args.cam2seg:
        tag = 'multiscale_lam2seg/mslam' if args.refine_with_multiscale else 'lam2seg_seeds/lam'
    else:
        tag = 'multiscale_lams/mslam' if args.refine_with_multiscale else 'lam_seeds/lam'
