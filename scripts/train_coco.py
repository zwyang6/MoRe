...
from utils import imutils,evaluate, optimizer
from utils.camutils import (cam_to_label, multi_scale_cam2, label_to_aff_mask, multi_scale_lam2,
                            refine_cams_with_bkg_v2,)
from utils.pyutils import AverageMeter, cal_eta, setup_logger, resume_train, save_train
...
parser.add_argument("--w_une", default=0.01, type=float, help="w_ctc")
parser.add_argument("--w_cle", default=0.1, type=float, help="w_ptc")
...
parser.add_argument("--num_attri", default=84, type=int, help="number of attribution tokens")
...
parser.add_argument("--cam2mask", default=50000, type=int, help="use mask from last layer")
...
### log utils
parser.add_argument("--resume_train", default=False, type=lambda x: x.lower() in ["true", "1", "yes"], help="log tb")
parser.add_argument("--resume_path", default="./resume.pth", type=str, help="log tb")
...
parser.add_argument("--log_tag", default="train_coco", type=str, help="train_voc")
...
def train(args=None):
    ...
    begin_iter = 0
    if args.resume_train and os.path.exists(args.resume_path):
        model, optim, begin_iter = resume_train(args,model,optimizer)
        if args.local_rank == 0:
            logging.info(f'Resuming_{begin_iter}iter_Training_from_{args.resume_train}...')
    ...
    for n_iter in range(begin_iter, args.max_iters):
        try:
            img_name, inputs, cls_label, img_box = next(train_loader_iter)
        except:
            train_sampler.set_epoch(np.random.randint(args.max_iters))
            train_loader_iter = iter(train_loader)
            img_name, inputs, cls_label, img_box = next(train_loader_iter)
        ...
        cls, segs, fmap, cls_aux, attr_tokens, score_maps = model(inputs)
        ...
        mask_cle = pseudo_label_cle if n_iter <= args.cam2mask else pseudo_label
        mask_une = pseudo_label_aux if n_iter <= args.cam2mask else pseudo_label
        CLE_loss = CRELoss_neg(temp=0.1).cuda()
        ...
        # warmup
        if n_iter <= 8000:
            loss = 1.0 * cls_loss + 1.0 * cls_loss_aux + 1.0 * attri_loss + 0.0 * ptc_loss + 0.0 * cle_loss + 0.0 * une_loss + 0.0 * seg_loss + 0.0 * reg_loss
        elif n_iter <= 12000:
            loss = 1.0 * cls_loss + 1.0 * cls_loss_aux + 1.0 * attri_loss + args.w_ptc * ptc_loss + 0.0 * cle_loss + 0.0 * une_loss + 0.0 * seg_loss + 0.0 * reg_loss
        elif n_iter <= 16000:
            loss = 1.0 * cls_loss + 1.0 * cls_loss_aux + 1.0 * attri_loss + args.w_ptc * ptc_loss + args.w_cle * cle_loss + args.w_une * une_loss + 0.0 * seg_loss + 0.0 * reg_loss
        else:
        ...
        if (n_iter + 1) % args.log_iters == 0:
            ...
            if args.local_rank == 0:
                logging.info("Iter: %d; Elasped: %s; ETA: %s; LR: %.3e; cls_loss: %.4f, cls_loss_aux: %.4f, cls_loss_mct: %.4f, ptc_loss: %.4f,  cle_loss: %.4f, une_loss: %.4f, seg_loss: %.4f..." % \
                    (n_iter + 1, delta, eta, cur_lr, avg_meter.pop('cls_loss'), avg_meter.pop('cls_loss_aux'), avg_meter.pop('attri_loss'),\
                    avg_meter.pop('ptc_loss'), avg_meter.pop('cle_loss'), avg_meter.pop('une_loss'), avg_meter.pop('seg_loss')))
        ...
        if (n_iter + 1) % 1000 == 0:
            ...
            if args.local_rank == 0 and args.save_ckpt:
                ...
                save_train(model,optim,ckpt_name,step_,cur_lr)

        ### save ckpt per eval_iters and validate
